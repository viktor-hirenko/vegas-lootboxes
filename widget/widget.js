...
import { MESSAGE_TYPES, DEFAULT_SKELETON_COUNT } from './modules/constants.js';
...
const headerEl = document.querySelector('[data-lb-header]');
const headingEl = document.querySelector('[data-lb-heading]');
const subtitleEl = document.querySelector('[data-lb-subtitle]');
...
const store = createContentStore({
  heading: initialParams.heading,
  subtitle: initialParams.subtitle,
...
function render() {
  ...
  const hasHeaderText = Boolean(state.heading || state.subtitle);
  if (headerEl) headerEl.style.display = hasHeaderText ? '' : 'none';
  if (headingEl) headingEl.textContent = state.heading;
