// Minimal reactive store for the widget's content (heading/subtitle/cards).
 ...
 * @property {'available'|'locked'|'prize'|'prediction'|'empty'} state
 * @property {string} date e.g. "1 Mar"
 * @property {string} title
 * @property {string} cta e.g. "Go to Bonuses" (prize state)
 * @property {string} tag e.g. "Opened" / "Missed"
 */
...
export function createContentStore(initial) {
  let state = { heading: '', subtitle: '', cards: [], ...initial };
  ...
  /** Shallow-merge patch into top-level state (heading/subtitle/cards). */
  function set(patch) {
